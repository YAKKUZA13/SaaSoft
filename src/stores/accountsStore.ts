...
export const useAccountsStore = defineStore('accounts', () => {
  ...
  const validateAccount = (account: Account): boolean => {
    const loginValue = account.login ?? ''
    const passwordValue = account.password ?? ''
    const hasLogin = loginValue.trim().length > 0 && loginValue.length <= 100
    const hasPassword = account.type === 'LDAP' || (passwordValue.trim().length > 0 && passwordValue.length <= 100)
